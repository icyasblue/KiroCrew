...
Retention reads those reasons as a **class**, not by name. `pinned_fs` splits every
reason code once: `NEVER_ARCHIVED` (`symlink`, `not_regular`) are screened on every run
by design, so a bundle that screened one is complete; everything else means the archive
