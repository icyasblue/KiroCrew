...
**Two empty states, because zero rows has two causes.** A token whose prefix
resolves out of the project lists nothing BY RULE, however full that directory is,
so the composer must not answer it with "No matching files" — that asserts
something false about a directory the user just named. `pathTokenLeavesProject`
(in `components/composerTokens.ts`) decides which of the two sentences to show
from the typed prefix and the project dir, as string math. It is deliberately NOT
a second containment check: the endpoint decides what may be listed, on realpaths,
and is the only thing that can — a symlink is invisible from the browser. The
client only picks the copy.
...
That refusal and "nothing is there" are ONE answer to the caller and TWO audit
facts, so the listing distinguishes them internally (`refused` vs `missing`) and
each writes its own SEL line — a swapped target is a `denied` record, an absent
directory an `allowed` one with zero results. Without the split, the interesting
case was the one that logged nothing.
