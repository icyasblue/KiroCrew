...
`slot.project` directory. The id names a Project registered in the fenced
...
mask). The slot's project directory is derived from that Project's primary
...
project directory and the root the review digest reads
`.kiro/settings/mcp.json` and `.kiro/agents/` from, so the tree a session runs in
and the tree the owner reviewed can never be different ones.
...
kiro-cli reads the session cwd's own `.kiro/settings/mcp.json` (full server
...
review step — so a hostile commit could otherwise land an executable definition
the next session start runs. Those two surfaces are therefore part of the
reviewed bundle: `project_review.py` digests them alongside `project.yaml`,
reading every file through the hardened link-refusing, byte-bounded reader, and
the digest plus its per-file hashes are recorded in the fenced registry
(`RegisteredProject.reviewed_digest` / `reviewed_files`) when the owner adds the
Project — an add IS the activation. Absent files hash as *absent* rather than
being skipped, so deleting a reviewed `mcp.json` is a change the owner is shown;
a checkout carrying neither surface has nothing to gate. Text surfaces are
deliberately excluded: `.kiro/steering` and the checkout's documents reach a
session under the same posture as a directory the user opened by hand, and
`.kiro/skills` keeps its own per-directory consent grant.

`sync` re-registers the clone but never re-records the review, so the digest is
...
fails the turn loudly, exactly as a removed Project does. `POST
/api/project-bundles/{id}/review` (owner-only, SEL-audited as a governance
decision) recomputes the digest from the current checkout and records it, which
returns the Project to healthy. A Project carrying no review record at all is
not treated as stale — every add path writes one, and the registry is fenced
from every agent, so a missing record cannot have been dropped by one.
