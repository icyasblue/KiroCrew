...
`owner_member` set AND that owner still an active agent EXCLUSIVELY bound to this
store), `private_v2_nonmember` (a private V2 store that is not currently a member
store — `owner_member` unset, OR its owner deleted / re-bound elsewhere),
`non_member` (`default`/empty, or a definitively NON-V2 store — a legacy V1 named
store), or `unknown` (the config read raised, `memory_stores` / the whole file
degraded away, OR a non-`default` store's record is absent — the loader silently
DROPS a malformed entry without degrading the section, so a missing record is
corruption, not "no such member"). The fence (`_store_binding_creator_fences`)
binds a caller whose store is `member`, `private_v2_nonmember`, OR `unknown`, and
leaves a `non_member` store unfenced.
