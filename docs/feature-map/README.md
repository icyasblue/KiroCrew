...
| Private member history tools | MCP-only member-scoped relay that renders `search_chat_history`, `get_chat_session` and `list_sessions` for a private member whose sandbox hides the transcript tree; covered by `test/test_private_member_history_tools.py` | Agent calls via the `kirocrew-core` history MCP tools; no dashboard page | — | `handlers/sessions.py` (`api_sessions_member_history`) | `POST /api/sessions/member-history` |
