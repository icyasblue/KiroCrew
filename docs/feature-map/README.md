...
| Projects | Portable Project bundles — register by URL or local path, inspect, sync a managed Git clone, remove; a Project's sessions and one-click session creation on it. The thin Project installs nothing, so there is no activate/deactivate step; the owner's one trust step is REVIEW over the primary checkout's own executable surfaces (`.kiro/settings/mcp.json`, `.kiro/agents/`), which a sync can move — a moved digest reports `review_stale` and refuses session start until re-reviewed | left-rail Projects entry | `pages/ProjectBundlesPage.tsx` | `dashboard/handlers_project.py`, `project_registry.py`, `project_git.py`, `project_manifest.py`, `project_review.py`, `project_sessions.py` | `GET,POST /api/project-bundles`, `GET,DELETE /api/project-bundles/{id}`, `POST /api/project-bundles/add`, `POST /api/project-bundles/{id}/sync`, `POST /api/project-bundles/{id}/review` |
