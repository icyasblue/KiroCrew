...
| Model picker order | User-authored display order for the model-selection dropdown, honored by every picker (chat composer, agents pages, sidebar bulk switcher, app pickers, cron job form); Auto stays pinned first, unlisted models append in backend order | Settings ▸ Chat → **Selectable models** — drag a row's grip to reorder (same rows as visibility), Reset to default order as the footer action below the list | `components/MultiSelect.tsx` (reorder mode), `pages/settings/ChatPanel.tsx`, `providers/modelList.ts` (`applyModelOrder`, `mergeReorderedNames`), `hooks/useAvailableModels.ts` | `handlers/core.py` (`api_kirocrew_config_patch`, `str_list`) | `GET,PATCH /api/config/kirocrew` (`agent.model_order`) |
