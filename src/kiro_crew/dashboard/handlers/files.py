...
def _complete_path_listing(
    ...
    or a non-directory sat at the target when it was opened) or ``"missing"``
    (nothing is there). The last two are one answer to the caller and two
    different audit facts, which is why they are separate statuses.
    ...
    # at the name, and separately for a directory that is simply not there. Both
