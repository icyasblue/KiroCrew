...
def _project_payload(
    ...
    return {
        ...
        "mcp": [
            {"name": _redact(server.name), "scope": _redact_json_value(server.scope)}
            for server in manifest.mcp
        ],
        "memory": {"mode": manifest.memory_mode},
        "registrations": registrations,
        "health": health,
        "sessions": sessions or [],
    }
...
def _record_review(registry: ProjectRegistry, project: RegisteredProject) -> RegisteredProject:
    """Record the current executable surfaces as the ones the owner reviewed.

    Adding a Project IS the owner's activation, so the record is written there;
    the ``/review`` route writes it again when the owner re-reviews a checkout
    that a sync moved. A checkout that cannot be resolved yet still records the
    manifest, so the first resolvable session start is measured against a real
...
def _materialize_and_review(
    ...
    different directory. Materialization failures are reported through health,
    never raised, so one unreachable URL leaves the Project registered.
...
async def api_project_review(request: web.Request) -> web.Response:
    """Record the Project's current executable surfaces as reviewed."""
    ...
    def _review() -> RegisteredProject:
        return _record_review(registry, registry.get(project_id))

    try:
        project = await asyncio.to_thread(_review)
    except ProjectRegistryError:
        return web.json_response(
            {"error": "project not found", "code": "project_not_found"}, status=404
        )
    ...
    try:
        await asyncio.to_thread(
            lambda: _sel().log_governance_decision(
                ...
                reason="operator reviewed the Project's executable surfaces",
                critical=True,
            )
        )
    except Exception:
        logger.error("SEL audit for Project review failed", exc_info=True)
    return web.json_response(await asyncio.to_thread(_project_payload, project, None, registry))
