...
def _member_store_verdict(store: str) -> str:
    ...
    * ``_STORE_PRIVATE_V2_NONMEMBER`` — a private V2 store (``memory_version ==
      2``) that is NOT (currently) a member store: either its ``owner_member`` is
      unset, or its owner is not an active agent EXCLUSIVELY bound to it (a
      deleted or re-bound owner). This is what a member store BECOMES when the
      member is un-assigned mid-await, and the fence must still bind such a caller
      (it was admitted as a member) even where it does not currently classify as
      one.
    * ``_STORE_NON_MEMBER`` — ``default``/empty, or a recorded NON-V2 store (a
      legacy V1 named store). Definitively not a private V2 store; the fence must
      NOT bind it, so a V1-named-store owner tab keeps its reach.
    * ``_STORE_UNKNOWN`` — the read could not decide: the config raised,
      ``memory_stores`` / the whole file degraded away, OR a NON-default store's
      record is absent (the loader silently DROPS a malformed entry without
      degrading the section, so a missing record is corruption, not "no such
      member"). The fence fails SAFE.
    """
    ...
    if record is None:
        # A NON-default store name (``default``/empty already returned above)
        # whose record is absent. This is NOT the same as "definitely not a member
        # store": the config loader SILENTLY DROPS a malformed ``memory_stores``
        # entry (a non-dict value) WITHOUT marking the section degraded, so a
        # corrupted member-store record lands here rather than at the
        # ``degraded_sections`` check above. Returning ``_STORE_NON_MEMBER`` would
        # leave that caller UNFENCED — an admitted member whose record was dropped
        # in an await window would fall through to ``bool(_created_by)`` and reach
        # a session it did not create. Fail SAFE to ``_STORE_UNKNOWN`` instead: the
        # gate never admits a caller whose scope resolved to nothing, so the only
        # caller reaching the fence on a missing non-default record was admitted as
        # a member, and keeping it creator-fenced is the same fail-safe direction
        # the degraded-section and read-error branches take.
        return _STORE_UNKNOWN
