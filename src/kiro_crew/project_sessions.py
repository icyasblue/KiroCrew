...
def review_stale_files(
    ...
    """Reviewed files whose current content differs from the review record.

    Empty means nothing to answer for: either the surfaces match what the owner
    reviewed, or the Project carries no review record at all. A Project has a
    record from the moment it is added, so an absent one is not a stale bundle
    to refuse -- it is a registry entry no add path wrote, and the registry is
    fenced from every agent, so the record cannot have been dropped by one.
    """
    if not project.reviewed_digest:
        return ()
    digest, hashes = compute_review_digest(bundle_dir, workspace_dir)
    unreviewable = unreviewable_files(hashes)
    if digest == project.reviewed_digest and not unreviewable:
        return ()
    return tuple(
        sorted(set(changed_review_files(project.reviewed_files, hashes)) | set(unreviewable))
    )
