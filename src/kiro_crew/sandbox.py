...
def sandbox_exec_argv(
    ...
    *,
    ...
    # The strict no-alias leaves must be checked HERE too, not only on Linux. A Seatbelt
    # `deny file-write*` is a rule about a PATHNAME, so it covers the link's own name and
    # says nothing about what the link resolves to: a symlinked `cloud.json` whose target
    # sits in a writable directory stays writable through the target, and a write there
    # picks the container image a Fargate launch runs. The same is true of a second
    # hardlink, whose other name the profile never mentions.
    #
    # This is only the alias REFUSAL, deliberately not `_materialize_sealable_ceilings()`:
    # that function also prepares bind-mount targets, which is Linux-specific work with no
    # meaning under Seatbelt. The neighbouring pre-create note explains why absent leaves
    # need nothing here -- a path rule does hold for a name that does not exist yet -- and
    # that argument is exactly what does NOT extend to a name that exists and is an alias.
