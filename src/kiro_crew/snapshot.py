...
def snapshot_main(
    ...
    # Asked as a CLASS, never by naming a reason code. Naming one is what let this
    # guard see `unreadable_entry` alone while `too_large`, `vanished` and
    # `identity_changed` walked past it into the prune: a guard that enumerates codes
    # is stale from the moment the next code is added, and it goes on answering the old
    # way until someone remembers to come back here. `pinned_fs.omits_wanted_data`
    # answers True for anything not explicitly screened-by-design, so a new reason is
    # incomplete-by-default and the surplus bundle is the cost of forgetting.
