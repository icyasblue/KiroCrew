...
class RegisteredProject:
    ...
    checkout's executable surfaces (``project_review.py``) -- recorded when the
    Project was added, which is the owner's activation. ``sync`` recomputes it
...
class ProjectRegistry:
        ...
        *,
        ...
        with self._lock(exclusive=True):
            ...
            if registration not in registrations:
                if current is not None:
                    if before_primary_change is None:
                        raise ProjectRegistryError(
                            "Project is already registered at another path; remove it first"
                        )
            ...
            project = RegisteredProject(
                ...
                # CARRIED FORWARD, deliberately: ``sync`` re-registers the same
                # clone to refresh its metadata, and rebuilding the entry without
                # the review record would silently mark every synced Project as
                # reviewed again -- which is the exact check ``sync`` exists to
                # trip. Only ``record_review`` advances it.
                reviewed_digest=current.reviewed_digest if current else "",
                reviewed_files=dict(current.reviewed_files) if current else {},
            )
    ...
    def record_review(
        ...
        The one writer of the review record. Called when the owner adds a
        Project (an add IS the activation) and when they explicitly re-review a
        Project whose synced checkout moved.
        """
        with self._lock(exclusive=True):
            projects = self._load_unlocked()
            try:
    ...
    def add_managed(
        ...
        *,
        ...
        return self._register(
            manifest,
            ProjectRegistration(
                path=bundle, origin="managed_git", remote=remote, default_branch=default_branch
            ),
            before_primary_change=before_primary_change,
        )
