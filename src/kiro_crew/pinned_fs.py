...
#: one is complete -- there is nothing the operator asked for that it lacks.
NEVER_ARCHIVED = frozenset({SKIP_SYMLINK, SKIP_NOT_REGULAR})
...
#: away between the listing and the copy.
OMITS_WANTED_DATA = frozenset(
    {SKIP_UNREADABLE_ENTRY, SKIP_TOO_LARGE, SKIP_VANISHED, SKIP_IDENTITY_CHANGED}
)

#: Every reason code this module reports. Asserted complete by a test, so a code added
#: later cannot skip being classified.
SKIP_REASONS = NEVER_ARCHIVED | OMITS_WANTED_DATA
...
def omits_wanted_data(reason: str) -> bool:
    ...
    the old answer -- which is how a retention guard came to see
    ``SKIP_UNREADABLE_ENTRY`` alone while ``SKIP_TOO_LARGE``, ``SKIP_VANISHED`` and
    ``SKIP_IDENTITY_CHANGED`` walked past it into a prune that deleted the last
    complete backup.
    ...
    it, and :data:`SKIP_REASONS` is what tells the author to choose a side.
    """
    return reason not in NEVER_ARCHIVED
        ...
        for entry in sorted(names):
            ...
            if _stat.S_ISLNK(st.st_mode):
                ...
                child_src = _open_child_dir(
                    src_fd, entry, path, on_skip, skip_unreadable=skip_unreadable
                )
