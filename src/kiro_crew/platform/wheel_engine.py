...
#: pip lists the releases it saw right before that verdict. A digit means
#: releases exist and none published a usable wheel; "(from versions: none)"
#: means the index does not carry the package at all, which is not a
#: platform verdict (a private mirror, a yanked release) -- cli.sh's
#: _report_pip_failure draws the same line.
_RELEASES_SEEN_RE = re.compile(r"Could not find a version that satisfies.*\(from versions: [0-9]")


def _no_wheel_packages(pip_text: str) -> list[str]:
    """The requirements pip could not satisfy from wheels, in pip's own order.

    Empty unless pip also listed at least one release it did see, so an index
    that carries no release of the package keeps pip's own words.
    """
    if not _RELEASES_SEEN_RE.search(pip_text):
        return []
    seen: list[str] = []
