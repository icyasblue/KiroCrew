...
OFF_CARD_SETS: Mapping[str, str] = {
    ...
    "ACP_BACKENDS_SELF_SERVED_ACP": (
        "whether the harness's own binary serves ACP or an adapter does. A launch-path "
        "detail: the session starts either way, and nothing a reader picks a harness for"
    ),
...
#: reading no agent spec of Crew's.
KIRO_FAMILY_MARKER_SET = "ACP_BACKENDS_KIRO_SLASH_COMMANDS"

#: The set the union lines actually read for the kiro family. Held to the marker
#: by a test so a future substitution cannot widen it silently.
KIRO_FAMILY_STAND_IN_SET = "ACP_BACKENDS_KIRO_SLASH_COMMANDS"
