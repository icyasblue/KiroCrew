...
class GitProjectStore:
    ...
    def sync_sources(
        ...
        """Refresh every declared repo source. Failures are reported, not raised."""
    ...
    def materialize_sources(
        ...
        materialized. A failure is REPORTED, never raised: an unreachable URL
        must leave the Project registered so the owner can correct the manifest
        and sync, rather than losing the registration to one bad source.
        """
    ...
    def sync(self, identifier: str) -> RegisteredProject:
        ...
        with self._lock(f"{project.id}-bundle"):
            ...
            synced = self.registry.add_managed(
                registration.path,
