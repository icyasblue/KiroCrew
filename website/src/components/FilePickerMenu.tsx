...
import { pathTokenLeavesProject, splitPathToken } from './composerTokens'
...
export default function FilePickerMenu({ query, anchorRef, open, onSelect, onClose, onFileOpen, project, pathMode = false, sendOnEnter = 'enter' }: Props) {
  ...
  // them is "nothing matched": a token whose prefix resolves out of the project
  // lists nothing BY RULE, however full that directory is. Judged on the live
  // token, like every other empty-state branch here.
  const outsideProject = pathMode && !!project && pathTokenLeavesProject(project, query)
