...
async function main() {
  ...
  await menu.getByText(/Outside the project/i).waitFor({ timeout: 10000 })
  await page.waitForTimeout(400)
  ...
  await composer.pressSequentially('notes/', { delay: 15 })
  ...
  if (!asked.includes('../notes/|')) problems.push('the re-entering parent run was never requested')
  if (problems.length) {
