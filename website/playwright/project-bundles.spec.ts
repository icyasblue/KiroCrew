...
test('adds a multi-repo thin Project and starts a usable attached session', async ({ page, request }) => {
  ...
  try {
    ...
    // The thin manifest declares intent only: repos (one primary), MCP servers
    // by NAME, and whether the Project gets its own memory. No `context` block
    ...
    writeFileSync(join(bundle, 'project.yaml'), JSON.stringify({
      ...
      sources: [
        { id: 'web', type: 'repo', url: webRemote, role: 'reference' },
        { id: 'service', type: 'repo', url: serviceRemote, role: 'primary' },
      ],
      mcp: [{ name: 'docs', scope: { space: 'LAUNCH' } }],
      memory: { mode: 'project' },
    }, null, 2), 'utf8')
    ...
    // Declared, not yet acted on: the MCP name and the memory mode are listed.
    await expect(page.getByText('Declared context', { exact: true })).toBeVisible()
    ...
    writeFileSync(join(bundle, 'project.yaml'), JSON.stringify({
      ...
      sources: [
        { id: 'web', type: 'repo', url: webRemote, default_branch: 'main', role: 'reference' },
        { id: 'service', type: 'repo', url: serviceRemote, default_branch: 'main', role: 'primary' },
        { id: 'docs', type: 'repo', url: docsRemote, default_branch: 'main', role: 'reference' },
      ],
      mcp: [{ name: 'docs', scope: { space: 'LAUNCH' } }, { name: 'datadog' }],
      memory: { mode: 'project' },
    }, null, 2), 'utf8')
