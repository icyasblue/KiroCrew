...
class TestBinaryOnlyDependencies:
    ...
    def test_an_index_with_no_release_keeps_pips_own_words(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
        """`(from versions: none)` is the index not carrying the package at all
        (a private mirror, a yanked release), not a host without a wheel: the
        same line cli.sh's _report_pip_failure draws."""
        monkeypatch.delenv("KIROCREW_ALLOW_SOURCE_BUILDS", raising=False)
        wheel = tmp_path / "w.whl"
        self._failing_install(
            monkeypatch,
    ...
    def test_no_wheel_packages_are_deduplicated_and_capped(self) -> None:
        ...
        # The verdict alone, with no release list, is not a platform verdict.
        assert (
            wheel_engine._no_wheel_packages("ERROR: No matching distribution found for pkg0") == []
        )
