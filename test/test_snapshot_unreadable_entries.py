...
class TestEveryOmissionReasonIsClassified:
    ...
    The guard used to name ``unreadable_entry``, so `too_large`, `vanished` and
    `identity_changed` walked past it into a prune that deleted the last complete
    ...
    def test_the_split_covers_every_reason_code_in_the_module(self):
        ...
        assert declared == pinned_fs.SKIP_REASONS, (
            "a reason code is not in NEVER_ARCHIVED or OMITS_WANTED_DATA: "
            f"{sorted(declared ^ pinned_fs.SKIP_REASONS)}"
        )

    def test_the_two_halves_do_not_overlap(self):
        assert not (pinned_fs.NEVER_ARCHIVED & pinned_fs.OMITS_WANTED_DATA)
