...
class TestMemberStoreVerdict:
    ...
    def test_missing_non_default_store_record_is_unknown(self):
        # A NON-default store name with no record fails SAFE to `unknown`, not
        # `non_member`: the loader silently DROPS a malformed `memory_stores`
        # entry without degrading the section, so a missing record is corruption,
        # and an admitted member whose record was dropped must stay fenced.
        with patch.object(sc.KiroCrewConfig, "load", return_value=self._cfg({})):
            assert sc._member_store_verdict("member-ghost-abc") == sc._STORE_UNKNOWN
            # The admission boolean still fails closed: unknown is not member-owned.
            assert sc._store_is_member_owned("member-ghost-abc") is False
