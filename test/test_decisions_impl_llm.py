...
def clean_registry():
    """Establish an unset registry, and leave one behind.

    Clearing on the way IN is not belt-and-braces: ``DashboardState.__init__``
    registers its ``SessionManager``, so any earlier test file that still holds a
    dashboard built with a mock leaves that mock in this module's global. A test
    asserting "unset means no lane" has to establish unset itself, or it passes
    only when it happens to run before such a file.
    """
